...
void Faction::load(const XmlNode *node, World *world, const FactionType *ft, ControlType control, TechTree *tt) {
	...
	upgradeManager.load(node->getChild("upgrades"), ft);
	...
	for(int i = 0; i < n->getChildCount(); ++i) {
		new Unit(n->getChild("unit", i), world, this, map, tt);
...
bool Faction::reqsOk(const CommandType *ct) const{
	...
	if(ct->getProduced()!=NULL && !reqsOk(ct->getProduced()) || !isCommandAvailable(ct)){
		return false;
	}
...
bool Faction::isCommandAvailable(const CommandType *ct) const {
