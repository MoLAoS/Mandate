...
CommandResult Commander::tryGiveCommand(const Unit* unit, const CommandType *commandType, CommandFlags flags, const Vec2i &pos, const UnitType* unitType) const{
	NetworkCommand networkCommand(nctGiveCommand, unit->getId(), commandType->getId(), flags, pos, unitType->getId());
	return pushNetworkCommand(&networkCommand);
}

CommandResult Commander::tryGiveCommand(const Selection *selection, CommandClass commandClass, CommandFlags flags, const Vec2i &pos, const Unit *targetUnit) const{
	...
	if(!selection->isEmpty()){
		...
		for(int i=0; i<selection->getCount(); ++i){
			...
			const CommandType *ct= unit->getFirstAvailableCt(commandClass);
			if(ct){
				...
				NetworkCommand networkCommand(nctGiveCommand, unitId, ct->getId(), flags, currPos, -1, targetId);
...
CommandResult Commander::tryGiveCommand(const Selection *selection, const CommandType *commandType, CommandFlags flags, const Vec2i &pos, const Unit *targetUnit) const{
	if(!selection->isEmpty() && commandType!=NULL){
		...
		for(int i=0; i<selection->getCount(); ++i){
			...
			int unitId= selection->getUnit(i)->getId();
			Vec2i currPos= computeDestPos(refPos, selection->getUnit(i)->getPos(), pos);
			NetworkCommand networkCommand(nctGiveCommand, unitId, commandType->getId(), flags, currPos, -1, targetId);
...
//auto command
CommandResult Commander::tryGiveCommand(const Selection *selection, CommandFlags flags, const Vec2i &pos, const Unit *targetUnit) const{
	if(!selection->isEmpty()){
		...
		for(int i=0; i<selection->getCount(); ++i){
			...
			else if(unit->getType()->hasMeetingPoint()) {
				trySetMeetingPoint(unit, flags, pos);
				results.push_back(crSuccess);
			} else {
				results.push_back(crFailUndefined);
			}
		}
...
void Commander::trySetMeetingPoint(const Unit* unit, CommandFlags flags, const Vec2i &pos) const{
	NetworkCommand command(nctSetMeetingPoint, unit->getId(), -1, flags, pos);
	pushNetworkCommand(&command);
}

void Commander::trySetAutoRepairEnabled(const Unit* unit, CommandFlags flags, bool enabled) const{
	NetworkCommand command(nctSetAutoRepair, unit->getId(), -1, flags, Vec2i(0), 0, (int)enabled);
	pushNetworkCommand(&command);
}
// ==================== PRIVATE ====================
	...
	if(unit!=NULL){
		switch(networkCommand->getNetworkCommandType()){
			...
			case nctSetMeetingPoint:
				unit->setMeetingPos(networkCommand->getPosition(), networkCommand->isQueue());
				break;
			case nctSetAutoRepair:
				unit->setAutoRepairEnabled(networkCommand->getTargetId() ? true : false);
				break;
			default:
				assert(false);
		}
...
Command* Commander::buildCommand(const NetworkCommand* networkCommand) const{
	...
	//validate unit
	if(!unit){
		throw runtime_error("Can not find unit with id: "
				+ intToStr(networkCommand->getUnitId()) + ". Game out of synch.");
	}
	...
	if(!ct){
		throw runtime_error("Can not find command type with id: "
				+ intToStr(networkCommand->getCommandTypeId()) + " in unit: "
				+ unit->getType()->getName() + ". Game out of synch.");
	}
	...
	//create command
	if(unitType) {
		return new Command(ct, networkCommand->getFlags(), networkCommand->getPosition(), unitType);
	} else if(target) {
		return new Command(ct, networkCommand->getFlags(), target);
	} else {
		return  new Command(ct, networkCommand->getFlags(), networkCommand->getPosition());
	}
}
