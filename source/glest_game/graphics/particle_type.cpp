...
void ParticleSystemType::load(const XmlNode *particleSystemNode, const string &dir){
	...
	speed= speedNode->getAttribute("value")->getFloatValue()/Config::getInstance().getWorldUpdateFPS();

	//gravity
	const XmlNode *gravityNode= particleSystemNode->getChild("gravity");
	gravity= gravityNode->getAttribute("value")->getFloatValue()/Config::getInstance().getWorldUpdateFPS();
