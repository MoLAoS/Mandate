...
void Ai::update(){
	...
	for(AiRules::iterator it= aiRules.begin(); it!=aiRules.end(); ++it){
		if((aiInterface->getTimer() % ((*it)->getTestInterval()*Config::getInstance().getWorldUpdateFPS()/1000))==0){
			if((*it)->test()){
				aiInterface->printLog(3, intToStr(1000*aiInterface->getTimer()/Config::getInstance().getWorldUpdateFPS()) + ": Executing rule: " + (*it)->getName() + '\n');
				(*it)->execute();
			}
		}
	}
...
bool Ai::findAbleUnit(int *unitIndex, CommandClass ability, bool idleOnly, bool autoCmd){
	vector<int> units;
	vector<int> autoCmdUnits;
	...
	for(int i=0; i<aiInterface->getMyUnitCount(); ++i){
		...
		if(unit->getType()->hasCommandClass(ability)) {
			if(!idleOnly || !unit->anyCommand() || unit->getCurrCommand()->getCommandType()->getClass()==ccStop){
				units.push_back(i);
			} else if(autoCmd && unit->getCurrCommand()->isAuto()) {
				autoCmdUnits.push_back(i);
			}
		}
	}

	if(units.empty()) {
		if(!autoCmdUnits.empty()) {
			*unitIndex = autoCmdUnits[random.randRange(0, autoCmdUnits.size()-1)];
			return true;
		}
		return false;
	} else {
