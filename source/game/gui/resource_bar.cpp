...
UnitBarFrame::UnitBarFrame()
		: Frame((Container*)WidgetWindow::getInstance(), ButtonFlags::SHRINK | ButtonFlags::EXPAND) {
	setWidgetStyle(WidgetType::GAME_WIDGET_FRAME);
	Frame::setTitleBarSize(20);
	m_unitBar = new UnitBar(this);
	CellStrip::addCells(1);
	m_unitBar->setCell(1);
	Anchors a(Anchor(AnchorType::RIGID, 0), Anchor(AnchorType::RIGID, 0),
		Anchor(AnchorType::NONE, 0), Anchor(AnchorType::NONE, 0));
	m_unitBar ->setAnchors(a);

	Expand.connect(this, &UnitBarFrame::onExpand);
	Shrink.connect(this, &UnitBarFrame::onShrink);
	doEnableShrinkExpand(16);
	setPinned(g_config.getUiPinWidgets());
}

void UnitBarFrame::setPinned(bool v) {
	Frame::setPinned(v);
	m_titleBar->showShrinkExpand(!v);
}

void UnitBarFrame::doEnableShrinkExpand(int sz) {
	switch (sz) {
		case 16:
			enableShrinkExpand(false, true);
			break;
...
void UnitBarFrame::onExpand(Widget*) {
	int sz = m_unitBar->getIconSize();
	assert(sz == 16 || sz == 24);
	sz += 8;
	m_unitBar->reInit(sz);
	doEnableShrinkExpand(sz);
...
void UnitBarFrame::onShrink(Widget*) {
	int sz = m_unitBar->getIconSize();
	assert(sz == 32 || sz == 24);
	sz -= 8;
	m_unitBar->reInit(sz);
...
UnitBar::UnitBar(Container *parent)
		: Widget(parent)
		, MouseWidget(this)
		, ImageWidget(this)
		, TextWidget(this)
...
void UnitBar::reInit(int iconSize) {
	Config &cfg = g_config;
	if (iconSize != -1) {
		assert(iconSize == 16  || iconSize == 24 || iconSize == 32);
		m_iconSize = iconSize;
	...
	const bool includeNames = cfg.getUiUnitNames();
	const int padding = m_iconSize / 4;
	const int imgAndPad = m_iconSize + padding;
	...
	const FontMetrics *fm = font->getMetrics();
	string trailerStrings = "8000";
	vector<int> reqWidths;
	int total_req = 0, i = 0;;
	foreach_const (vector<const UnitType*>, it, m_unitTypes) {
    string testLine = (includeNames ? m_headerStrings[i] : string("")) + trailerStrings;
		int w = imgAndPad + int(fm->getTextDiminsions(testLine).w);
		total_req += w;
		reqWidths.push_back(w);
		++i;
	...
	Vec2i pos;
	if (cfg.getUiLastUnitBarPosX() != -1 && cfg.getUiLastUnitBarPosY() != -1) {
		pos = Vec2i(cfg.getUiLastUnitBarPosX(), cfg.getUiLastUnitBarPosY());
	} else {
		pos = Vec2i(g_metrics.getScreenW() / 2 - m_parent->getWidth() / 2, 5);
	}
	m_parent->setPos(pos);
...
void UnitBar::init(const Faction *faction, std::set<const UnitType*> &types) {
	m_faction = faction;
	...
	m_iconSize = (g_config.getUiLastUnitBarSize() + 1) * 8;
	reInit(m_iconSize);
}

void UnitBar::persist() {
	Config &cfg = g_config;

	Vec2i pos = m_parent->getPos();
	int sz = m_iconSize / 8 - 1;

	cfg.setUiLastUnitBarSize(sz);
	cfg.setUiLastUnitBarPosX(pos.x);
	cfg.setUiLastUnitBarPosY(pos.y);
}

void UnitBar::reset() {
	Config &cfg = g_config;
	cfg.setUiLastUnitBarSize(1);
	cfg.setUiLastUnitBarPosX(-1);
	cfg.setUiLastUnitBarPosY(-1);
	reInit(16);
}
...
void UnitBar::update() {
	if (m_updateCounter % 10 == 0) {
		for (int i=0; i < m_unitTypes.size(); ++i) {
			stringstream ss;
			stringstream css;
			const UnitType* &ut = m_unitTypes[i];
			if (g_config.getUiUnitNames()) {
				ss << m_headerStrings[i];
			}
            ss << m_faction->getCountOfUnitType(ut);
			setText(ss.str(), i);
		}
	}
	++m_updateCounter;
...
bool UnitBar::mouseDown(MouseButton btn, Vec2i pos) {
	return false;
}

bool UnitBar::mouseUp(MouseButton btn, Vec2i pos) {
	return false;
}

bool UnitBar::mouseMove(Vec2i pos) {
	return false;
}
