...
void MinimapFrame::initMinimp(MinimapSize size, int w, int h, const World *world, bool resumingGame) {
	...
	if (size == MinimapSize::SMALL) {
		onShrink(0);
	} else if (size == MinimapSize::LARGE) {
		onExpand(0);
	}
}
...
void Minimap::reset() {
	...
	setMinimapSize(MinimapSize::MEDIUM);
