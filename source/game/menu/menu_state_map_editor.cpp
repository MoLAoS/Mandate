...
MenuStateMapEditor::MenuStateMapEditor(Program &program, MainMenu *mainMenu, bool openNetworkSlots)
...
//	_PROFILE_FUNCTION();
	...
	g_simInterface.getGameSettings().enableMapEditor();
	...
	m_randomLocsCheckbox->Clicked.connect(this, &MenuStateMapEditor::onCheckChanged);
	...
	m_SODCheckbox->Clicked.connect(this, &MenuStateMapEditor::onCheckChanged);
	...
	m_FOWCheckbox->Clicked.connect(this, &MenuStateMapEditor::onCheckChanged);
	...
	m_mapList->SelectionChanged.connect(this, &MenuStateMapEditor::onChangeMap);
	...
	m_tilesetList->SelectionChanged.connect(this, &MenuStateMapEditor::onChangeTileset);
	...
	m_techTreeList->SelectionChanged.connect(this, &MenuStateMapEditor::onChangeTechtree);
	...
	m_returnButton->Clicked.connect(this, &MenuStateMapEditor::onButtonClick);

	m_playNow = new Button(strip, Vec2i(0), Vec2i(w, h));
	m_playNow->setCell(1);
	m_playNow->setAnchors(a3);
	m_playNow->setText(lang.get("PlayNow"));
	m_playNow->Clicked.connect(this, &MenuStateMapEditor::onButtonClick);
	...
	for (int i = 0; i < GameConstants::maxPlayers; ++i) {
		m_playerSlots[i]->ControlChanged.connect(this, &MenuStateMapEditor::onChangeControl);
		m_playerSlots[i]->FactionChanged.connect(this, &MenuStateMapEditor::onChangeFaction);
		m_playerSlots[i]->TeamChanged.connect(this, &MenuStateMapEditor::onChangeTeam);
		m_playerSlots[i]->ColourChanged.connect(this, &MenuStateMapEditor::onChangeColour);
	}
	program.setFade(0.f);
}

MenuStateMapEditor::~MenuStateMapEditor() {
}

//  === util ===

bool MenuStateMapEditor::getMapList() {
	vector<string> results;
	set<string> mapFiles;
...
int MenuStateMapEditor::getSlotIndex(PlayerSlotWidget* psw, PlayerSlotWidget* *slots) {
	for (int i=0; i < GameConstants::maxPlayers; ++i) {
		if (psw == slots[i]) {
			return i;
		}
...
int MenuStateMapEditor::getLowestFreeColourIndex(PlayerSlotWidget* *slots) {
...
void MenuStateMapEditor::onChangeFaction(Widget *source) {
...
void MenuStateMapEditor::onChangeControl(Widget *source) {
	PlayerSlotWidget *psw = static_cast<PlayerSlotWidget*>(source);
...
void MenuStateMapEditor::onChangeTeam(Widget *source) {
	PlayerSlotWidget *psw = static_cast<PlayerSlotWidget*>(source);
	GameSettings &gs = g_simInterface.getGameSettings();
	int ndx = getSlotIndex(psw, m_playerSlots);
	assert(ndx >= 0 && ndx < GameConstants::maxPlayers);
...
void MenuStateMapEditor::onChangeColour(Widget *source) {
	PlayerSlotWidget *psw = static_cast<PlayerSlotWidget*>(source);
	GameSettings &gs = g_simInterface.getGameSettings();
	int ndx = getSlotIndex(psw, m_playerSlots);
	assert(ndx >= 0 && ndx < GameConstants::maxPlayers);
...
void MenuStateMapEditor::onCheckChanged(Widget* cb) {
	GameSettings &gs = g_simInterface.getGameSettings();
	if (cb == m_FOWCheckbox) {
...
void MenuStateMapEditor::onChangeMap(Widget*) {
...
void MenuStateMapEditor::onChangeTileset(Widget*) {
	GameSettings &gs = g_simInterface.getGameSettings();
	assert(m_tilesetList->getSelectedIndex() >= 0);
	gs.setTilesetPath(string("tilesets/") + m_tilesetFiles[m_tilesetList->getSelectedIndex()]);
}

void MenuStateMapEditor::onChangeTechtree(Widget*) {
	reloadFactions(true);
}

void MenuStateMapEditor::onButtonClick(Widget *source) {
	...
	if (btn == m_returnButton) {
		...
		if (hasUnconnectedSlots()) {
			...
			m_messageDialog->Button1Clicked.connect(this, &MenuStateMapEditor::onCloseUnconnectedSlots);
			m_messageDialog->Button2Clicked.connect(this, &MenuStateMapEditor::onDismissDialog);
			m_messageDialog->Close.connect(this, &MenuStateMapEditor::onDismissDialog);
...
void MenuStateMapEditor::onDismissDialog(Widget*) {
	program.removeFloatingWidget(m_messageDialog);
	doFadeIn();
}

void MenuStateMapEditor::onCloseUnconnectedSlots(Widget*) {
...
void MenuStateMapEditor::update() {
	MenuState::update();
...
void MenuStateMapEditor::reloadFactions(bool setStagger) {
	GameSettings &gs = g_simInterface.getGameSettings();
	vector<string> results;
...
bool MenuStateMapEditor::loadGameSettings() {
...
void MenuStateMapEditor::updateControlers() {
	GameSettings &gs = g_simInterface.getGameSettings();
	assert(m_humanSlot >= 0);
...
bool MenuStateMapEditor::hasUnconnectedSlots() {
	ServerInterface* serverInterface = g_simInterface.asServerInterface();
	for (int i = 0; i < m_mapInfo.players; ++i) {
		if (m_playerSlots[i]->getControlType() == ControlType::NETWORK) {
...
bool MenuStateMapEditor::hasNetworkSlots() {
	for (int i = 0; i < m_mapInfo.players; ++i) {
		if (m_playerSlots[i]->getControlType() == ControlType::NETWORK) {
			return true;
		}
...
void MenuStateMapEditor::updateNetworkSlots() {
	ServerInterface* serverInterface = g_simInterface.asServerInterface();
	for (int i = 0; i < GameConstants::maxPlayers; ++i) {
...
void MenuStateMapEditor::randomiseStartLocs() {
	GameSettings &gs = g_simInterface.getGameSettings();
