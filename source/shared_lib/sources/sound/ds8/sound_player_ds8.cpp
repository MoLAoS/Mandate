...
long dsVolume(float floatVolume){
	...
	// (ln(x) + 2e) / (2e) gives a nicer range.
	const float two_e = 5.43656366f;
	float correctedVol = (logf(floatVolume) + two_e) / two_e;
	int vol= int(DSBVOLUME_MIN+correctedVol*(DSBVOLUME_MAX-(DSBVOLUME_MIN)));
